...
@section('content')
    <!-- form start -->
    <form method="POST" action="{{ route('sliders.update', base64_encode($slider->id)) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <x-admin::action-buttons :cancel-url="route('sliders.index')" save-label="Update" />
            <div class="col-md-8">
                <div class="card">
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control numeric-input" id="sort_order"
                                        name="sort_order" value="{{ $slider->sort_order }}">
                                    <span class="error-block"></span>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-select" name="status">
                                        <option value="1" @selected($slider->status == 1)>Enabled</option>
                                        <option value="0" @selected($slider->status == 0)>Disabled</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-select" id="action_type" name="action_type" required>
                                        <option value="none" @selected($slider->action_type === 'none')>None</option>
                                        <option value="url" @selected($slider->action_type === 'url')>URL</option>
                                    </select>
                                    <span class="error-block"></span>
                                </div>
                            </div>
                            <div class="col-md-4 action-fields" style="display: none;">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="action_title" name="action_title"
                                        value="{{ $slider->action_title }}" required>
                                    <span class="error-block"></span>
                                </div>
                            </div>
                            <div class="col-md-4 action-fields" style="display: none;">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="action_url" name="action_url"
                                        value="{{ $slider->action_url }}" required>
                                    <span class="error-block"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-3">
                                ...
                                <select class="form-select" id="media_type" name="media_type" required>
                                    <option value="image" @selected($slider->media_type === 'image')>Image</option>
                                    <option value="video" @selected($slider->media_type === 'video')>Video</option>
                                    <option value="video_url" @selected($slider->media_type === 'video_url')>Video URL</option>
                                </select>
                                <span class="error-block"></span>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12" id="video-url-field" style="display: none;">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control" id="video_url" name="video_url"
                                    value="{{ $slider->video_url }}" required>
                                <span class="error-block"></span>
                            </div>
                        </div>
                    </div>
        ...
        $(document).ready(function() {
            ...
            $('#image').filepond({
                ...
                @if ($slider->hasMedia('image'))
                    files: [{
                        source: '{{ $slider->getFirstMediaUrl('image') }}',
            ...
            $('#image_mobile').filepond({
                ...
                acceptedFileTypes: ['image/jpeg', 'image/png', 'image/jpg', 'image/webp'],
                maxFileSize: '300KB',
                @if ($slider->hasMedia('image_mobile'))
                    files: [{
                        source: '{{ $slider->getFirstMediaUrl('image_mobile') }}',
            ...
            $('#video_thumbnail_image').filepond({
                ...
                acceptedFileTypes: ['image/jpeg', 'image/png', 'image/jpg', 'image/webp'],
                maxFileSize: '300KB',
                @if ($slider->hasMedia('video_thumbnail_image'))
                    files: [{
                        source: '{{ $slider->getFirstMediaUrl('video_thumbnail_image') }}',
            ...
            $('#video').filepond({
                ...
                maxFileSize: '5MB',
                @if ($slider->hasMedia('video'))
                    files: [{
                        source: '{{ $slider->getFirstMediaUrl('video') }}',
            ...
            $('#video_url_thumbnail_image').filepond({
                ...
                acceptedFileTypes: ['image/jpeg', 'image/png', 'image/jpg', 'image/webp'],
                maxFileSize: '300KB',
                @if ($slider->hasMedia('video_url_thumbnail_image'))
                    files: [{
                        source: '{{ $slider->getFirstMediaUrl('video_url_thumbnail_image') }}',
