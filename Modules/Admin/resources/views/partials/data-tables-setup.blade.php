...
@push('js')
    ...
    <script>
        ...
        function initializeDataTable(url, columns, options = {}, id = "#dataTable") {
            const defaults = {
                processing: true,
                serverSide: true,
                paging: true,
            ...
            $(id).DataTable($.extend({}, defaults, options));
        }
        ...
        (function($) {
            $.sanitize = function(input) {
                // Check if input is null or undefined and return an empty string
                if (!input) return '';
                ...
                if (/script/i.test(decodedInput)) {
                    return '';
                }

                // Remove all HTML tags and replace &nbsp; with a space
                return decodedInput
