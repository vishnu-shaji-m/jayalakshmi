                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-3">
                                    ...
                                    @elseif ($adminSettings->type == 2)
                                        <input type="file" class="form-control" name="value" id="value" required>
                                    @endif
                                    @if ($adminSettings->key == 'website-auth-background')
                                        <div class="text-muted">
                                            Allowed formats: JPEG, WEBP.
                                        </div>
...
@push('js')
    <script>
        $(document).ready(function() {
            @if ($adminSettings->type == 2)
                var fileType = '{{ $adminSettings->key }}';
                var acceptedFileTypes = {
                    'website-auth-background': ['image/jpeg', 'image/webp'],
                    'website-logo': ['image/svg+xml'],
                    'website-favicon': ['image/vnd.microsoft.icon', 'image/x-icon']
                } [fileType] || ['image/*'];
                ...
                $('#value').filepond({
                    ...
                    maxFileSize: '300KB',
                    @if ($adminSettings->hasMedia('value'))
                        files: [{
                            source: '{{ $adminSettings->getFirstMediaUrl('value') }}'
                        }]
                    @endif
                });
            @endif
